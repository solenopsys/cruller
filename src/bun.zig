...
// bzrt-cut: pub const ci = @import("./cli/ci_info.zig");
...
// bzrt-cut: pub const darwin = @import("./platform/darwin.zig");
...
// bzrt-cut: pub const patch = @import("./patch/patch.zig");
pub const ini = @import("./ini/ini.zig");
pub const bits = @import("./meta/bits.zig");
// bzrt-cut: pub const css = @import("./css/css_parser.zig");
// bzrt-cut: pub const SmallList = css.SmallList;
pub const csrf = @import("./csrf/csrf.zig");
pub const compat = @import("./bun_core/bzrt_compat.zig");
pub const validators = @import("./runtime/node/util/validators.zig");

// bzrt-cut: pub const shell = @import("./shell/shell.zig");
// bzrt-cut: pub const md = @import("./md/root.zig");
...
// bzrt-cut: pub const libarchive = @import("./libarchive/libarchive.zig");
        ...
        .pointer => |ptr_info| {
            ...
            switch (ptr_info.size) {
                .one => switch (@typeInfo(ptr_info.child)) {
                    .array => |info| return @Pointer(.slice, attrs, info.child, info.sentinel()),
                    else => @compileError("invalid type given to std.mem.Span"),
                },
                .c => return @Pointer(.slice, attrs, ptr_info.child, 0),
                .many, .slice => return @Pointer(.slice, attrs, ptr_info.child, ptr_info.sentinel()),
            }
        },
...
// bzrt-cut: pub const Bunfig = @import("./cli/bunfig.zig").Bunfig;
...
pub fn StringArrayHashMap(comptime Type: type) type {
    return compat.ArrayHashMapManaged([]const u8, Type, StringArrayHashMapContext, true);
}

pub fn CaseInsensitiveASCIIStringArrayHashMap(comptime Type: type) type {
    return compat.ArrayHashMapManaged([]const u8, Type, CaseInsensitiveASCIIStringContext, true);
}
...
// bzrt-cut: pub const RunCommand = @import("./cli/run_command.zig").RunCommand;
...
pub fn selfExePath() ![:0]u8 {
    const memo = struct {
        ...
        pub fn load() ![:0]u8 {
            const n = try std.process.executablePath(compat.io(), &value);
            @This().len = n;
            value[@This().len] = 0;
            set = true;
...
pub fn getThreadCount() u16 {
    ...
    const ThreadCount = struct {
        ...
        var cached_thread_count_once = compat.once(getThreadCountOnce);
...
// bzrt-cut: pub const macho = @import("./exe_format/macho.zig");
// bzrt-cut: pub const pe = @import("./exe_format/pe.zig");
...
// bzrt-cut: pub const ConfigVersion = @import("./install/ConfigVersion.zig").ConfigVersion;
