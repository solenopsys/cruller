...
pub fn RawRefCount(comptime Int: type, comptime thread_safety: ThreadSafety) type {
    return struct {
        ...
        priv_thread_lock: if (thread_safety == .single_threaded) bun.safety.ThreadLock else void,
        ...
        pub fn init(initial_count: Int) Self {
            return .{
                ...
                .priv_thread_lock = switch (comptime thread_safety) {
        ...
        pub fn increment(self: *Self) void {
            switch (comptime thread_safety) {
                .single_threaded => {
                    self.priv_thread_lock.lockOrAssert();
                    self.raw_value += 1;
                },
                .thread_safe => {
