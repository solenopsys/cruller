...
priv_spawn_sync_event_loop: bun.ptr.Owned(?*SpawnSyncEventLoop) = .initNull(),
    ...
    return rare.stdin_store orelse brk: {
        ...
        const store = Blob.Store.new(.{
            ...
            .data = .{
                .file = .{
                    .pathlike = .{ .fd = fd },
                    .is_atty = if (fd.unwrapValid()) |valid| bun.compat.isatty(valid.native()) else false,
                    .mode = mode,
                },
            },
        });
...
pub fn deinit(this: *RareData) void {
    ...
    this.priv_spawn_sync_event_loop.deinit();
...
pub fn spawnSyncEventLoop(this: *RareData, vm: *jsc.VirtualMachine) *SpawnSyncEventLoop {
    return this.priv_spawn_sync_event_loop.get() orelse brk: {
        this.priv_spawn_sync_event_loop = .new(undefined);
        const ptr: *SpawnSyncEventLoop = this.priv_spawn_sync_event_loop.get().?;
        ptr.init(vm);
        break :brk ptr;
    };
}
