...
pub const FD = packed struct(backing_int) {
    ...
    pub fn cwd() FD {
        return .fromNative(std.posix.AT.FDCWD);
    }
    ...
    pub fn format(fd: FD, writer: *std.Io.Writer) std.Io.Writer.Error!void {
        ...
        switch (os) {
                ...
                if (Environment.isDebug and fd_native >= 3) print_with_path: {
                    ...
                    const path = getFdPathViaProc(fd_native, &path_buf) catch |err| switch (err) {
                        error.FileNotFound => {
...
const uv_file = i32; // bzrt: linux-only
...
fn getFdPathViaProc(fd_native: anytype, buf: *[std.fs.max_path_bytes]u8) error{FileNotFound}![]u8 {
    ...
    const rc = std.os.linux.readlink(proc, buf, buf.len);
