...
pub const PackedFeatures = brk: {
    ...
    for (packed_features_list) |name| {
        fields[i] = .{ .name = name, .type = bool, .default_value_ptr = &false, .is_comptime = false, .alignment = 0 };
        i += 1;
    }
    while (i < fields.len) : (i += 1) {
        fields[i] = .{ .name = std.fmt.comptimePrint("_{d}", .{i}), .type = bool, .default_value_ptr = &false, .is_comptime = false, .alignment = 0 };
    }
    const fields_c = fields;
    break :brk bun.meta.ReifyStruct(.@"packed", u64, &fields_c);
};
...
pub const GenerateHeader = struct {
    pub const GeneratePlatform = struct {
        ...
        var run_once = bun.compat.once(struct {
            fn run() void {
        ...
        var detectUseMsgXOnMacOS14OrLater_once = bun.compat.once(detectUseMsgXOnMacOS14OrLater);
