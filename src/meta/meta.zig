...
pub fn Tagged(comptime U: type, comptime T: type) type {
    return ReifyUnion(.auto, T, @typeInfo(U).@"union".fields);
}
...
fn VoidFields(comptime T: type) type {
    ...
    const new_fields_c = new_fields;
    return ReifyStruct(.auto, null, &new_fields_c);
}
...
pub fn ReifyStruct(
    ...
    for (fields, 0..) |f, i| {
        ...
        attrs[i] = .{
            ...
            .@"align" = if (f.alignment) |a| (if (a == 0) null else a) else null,
    ...
    const types_c = types;
    const attrs_c = attrs;
    return @Struct(layout, BackingInt, &names_c, &types_c, &attrs_c);
}
...
pub fn ReifyUnion(
    ...
    for (fields, 0..) |f, i| {
        names[i] = f.name;
        types[i] = f.type;
        attrs[i] = .{ .@"align" = if (f.alignment) |a| (if (a == 0) null else a) else null };
    }
    const names_c = names;
    const types_c = types;
    const attrs_c = attrs;
    return @Union(layout, Tag, &names_c, &types_c, &attrs_c);
}
