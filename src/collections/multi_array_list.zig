...
pub fn MultiArrayList(comptime T: type) type {
    return struct {
        ...
        const Elem = switch (@typeInfo(T)) {
            .@"struct" => T,
            .@"union" => |u| struct {
                pub const Bare = @import("bun").meta.ReifyUnion(u.layout, null, u.fields);
                pub const Tag =
        ...
        pub fn deinit(self: *Self, gpa: Allocator) void {
            self.priv_allocator.assertEq(gpa);
        ...
        pub fn addOne(self: *Self, allocator: Allocator) Allocator.Error!usize {
            self.priv_allocator.set(allocator);
        ...
        const Entry = entry: {
            ...
            const entry_fields_c = entry_fields;
            break :entry @import("bun").meta.ReifyStruct(.@"extern", null, &entry_fields_c);
        };
        /// This function is used in the debugger pretty formatters in tools/ to fetch the
        /// child field order and entry type to facilitate fancy debug printing for this type.
