    ...
    pub fn hash(
        ...
        switch (algorithm) {
            inline .argon2i, .argon2d, .argon2id => |argon| {
                ...
                const out_bytes = try pwhash.argon2.strHash(password, hash_options, &outbuf, bun.compat.io());
            ...
            .bcrypt => |cost| {
                ...
                const out_bytes = try pwhash.bcrypt.strHash(password_to_use, hash_options, outbuf_slice, bun.compat.io());
                return try allocator.dupe(u8, out_bytes);
            },
        }
    }
