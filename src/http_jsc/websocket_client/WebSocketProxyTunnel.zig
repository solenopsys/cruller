...
pub fn init(
    ...
    return bun.new(WebSocketProxyTunnel, .{
        .ref_count = .init(),
        .priv_upgrade_client = if (comptime ssl) .{ .https = upgrade_client } else .{ .http = upgrade_client },
        .priv_socket = if (comptime ssl) .{ .ssl = socket } else .{ .tcp = socket },
        .priv_sni_hostname = try bun.default_allocator.dupe(u8, sni_hostname),
        .priv_reject_unauthorized = reject_unauthorized,
    });
}

fn deinit(this: *WebSocketProxyTunnel) void {
    if (this.priv_wrapper) |*wrapper| {
        wrapper.deinit();
        this.priv_wrapper = null;
    }
    this.priv_write_buffer.deinit();
    if (this.priv_sni_hostname) |hostname| {
        bun.default_allocator.free(hostname);
        this.priv_sni_hostname = null;
    }
    bun.destroy(this);
}
...
pub fn start(this: *WebSocketProxyTunnel, ssl_options: SSLConfig, initial_data: []const u8) !void {
    ...
    // check uses this.priv_reject_unauthorized field.
    const options = ssl_options.forClientVerification();

    this.priv_wrapper = try SSLWrapperType.init(options, true, .{
        .ctx = this,
        .onOpen = onOpen,
        .onData = onData,
    ...
    if (initial_data.len > 0) {
        this.priv_wrapper.?.startWithPayload(initial_data);
    } else {
        this.priv_wrapper.?.start();
    }
}
...
fn onHandshake(this: *WebSocketProxyTunnel, success: bool, ssl_error: uws.us_bun_verify_error_t) void {
    ...
    if (this.priv_upgrade_client.isNone()) return;

    if (!success) {
        this.priv_upgrade_client.terminate(ErrorCode.tls_handshake_failed);
        return;
    }

    // Check for SSL errors if we need to reject unauthorized
    if (this.priv_reject_unauthorized) {
    ...
    this.priv_upgrade_client.onProxyTLSHandshakeComplete();
}
...
fn onClose(this: *WebSocketProxyTunnel) void {
    ...
    this.priv_upgrade_client.terminate(ErrorCode.ended);
}
...
pub fn clearConnectedWebSocket(this: *WebSocketProxyTunnel) void {
    this.priv_connected_websocket = null;
}
...
fn writeEncrypted(this: *WebSocketProxyTunnel, encrypted_data: []const u8) void {
    ...
    if (this.priv_write_buffer.isNotEmpty()) {
        bun.handleOom(this.priv_write_buffer.write(encrypted_data));
        return;
    }

    // Try direct write to socket
    const written = this.priv_socket.write(encrypted_data);
    if (written < 0) {
    ...
    if (written_usize < encrypted_data.len) {
        bun.handleOom(this.priv_write_buffer.write(encrypted_data[written_usize..]));
    }
}
...
pub fn onWritable(this: *WebSocketProxyTunnel) void {
    ...
    const to_send = this.priv_write_buffer.slice();
    if (to_send.len > 0) {
        const written = this.priv_socket.write(to_send);
        if (written < 0) return;

        const written_usize: usize = @intCast(written);
        if (written_usize == to_send.len) {
            this.priv_write_buffer.reset();
        } else {
            this.priv_write_buffer.cursor += written_usize;
...
pub fn hasBackpressure(this: *const WebSocketProxyTunnel) bool {
    return this.priv_write_buffer.isNotEmpty();
}
