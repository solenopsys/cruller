...
pub fn start(self: *Progress, name: []const u8, estimated_total_items: usize) *Node {
    ...
    self.timer = bun.compat.Timer.start() catch null;
    self.done = false;
    return &self.root;
}
...
fn maybeRefreshWithHeldLock(self: *Progress, timer: *bun.compat.Timer) void {
...
fn refreshWithHeldLock(self: *Progress) void {
    ...
    _ = file.writeStreamingAll(bun.compat.io(), self.output_buffer[0..end]) catch {
        // stop trying to write to this file
        self.terminal = null;
    };
    if (self.timer) |*timer| {
