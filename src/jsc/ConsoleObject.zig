...
pub const TablePrinter = struct {
    ...
    const VisibleCharacterCounter = struct {
        ...
        pub const Writer = bun.compat.GenericWriter(
...
const PendingTimers = std.AutoHashMap(u64, ?bun.compat.Timer);
...
pub fn time(
    ...
    if (!result.found_existing or (result.found_existing and result.value_ptr.* == null)) {
        result.value_ptr.* = bun.compat.Timer.start() catch unreachable;
    }
}
pub fn timeEnd(
    // console
    ...
    var value: bun.compat.Timer = result.value orelse return;
...
pub fn timeLog(
    ...
    const id = bun.hash(chars[0..len]);
    var value: bun.compat.Timer = (pending_time_logs.get(id) orelse return) orelse return;
    // get the duration in microseconds
    // then display it in milliseconds
    Output.printElapsed(@as(f64, @floatFromInt(value.read() / std.time.ns_per_us)) / std.time.us_per_ms);
    switch (len) {
