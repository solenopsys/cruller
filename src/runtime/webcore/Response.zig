...
priv_body: Body,
priv_init: Init,
priv_url: bun.String = bun.String.empty,
priv_redirected: bool = false,
...
priv_js_ref: jsc.JSRef = .empty(),

// We must report a consistent value for this
priv_reported_estimated_size: usize = 0,
...
pub fn init(response_init: Init, body: Body, url: bun.String, redirected: bool) Response {
    return Response{
        .priv_init = response_init,
        .priv_body = body,
        .priv_url = url,
        .priv_redirected = redirected,
    };
}

pub inline fn setInit(this: *Response, method: Method, status_code: u16, status_text: bun.String) void {
    this.priv_init.method = method;
    this.priv_init.status_code = status_code;
    this.priv_init.status_text.deref();
    this.priv_init.status_text = status_text;
}
pub inline fn setInitHeaders(this: *Response, headers: ?*FetchHeaders) void {
    if (this.priv_init.headers) |_headers| {
        _headers.deref();
    }
    this.priv_init.headers = headers;
}
pub inline fn getInitStatusCode(this: *Response) u16 {
    return this.priv_init.status_code;
}
pub inline fn getInitStatusText(this: *Response) bun.String {
    return this.priv_init.status_text;
}
...
pub fn estimatedSize(this: *Response) callconv(.c) usize {
    return this.priv_reported_estimated_size;
}

pub fn calculateEstimatedByteSize(this: *Response) void {
    this.priv_reported_estimated_size = this.priv_body.value.estimatedSize() +
        this.priv_url.byteSlice().len +
        this.priv_init.status_text.byteSlice().len +
        @sizeOf(Response);
}

fn checkBodyStreamRef(this: *Response, globalObject: *JSGlobalObject) void {
    if (this.priv_js_ref.tryGet()) |js_value| {
        if (this.priv_body.value == .Locked) {
            if (this.priv_body.value.Locked.readable.get(globalObject)) |stream| {
                ...
                this.priv_body.value.Locked.readable.deinit();
                this.priv_body.value.Locked.readable = .{};
            }
        }
    }
}
pub fn toJS(this: *Response, globalObject: *JSGlobalObject) JSValue {
    ...
    this.priv_js_ref = .initWeak(js_value);
...
pub inline fn getBodyReadableStream(
    ...
    if (this.priv_js_ref.tryGet()) |js_ref| {
    ...
    if (this.priv_body.value == .Locked) {
        return this.priv_body.value.Locked.readable.get(globalObject);
    }
    return null;
}
pub inline fn detachReadableStream(this: *Response, globalObject: *jsc.JSGlobalObject) void {
    ...
    if (this.priv_body.value == .Locked) {
        var old = this.priv_body.value.Locked.readable;
        old.deinit();
        this.priv_body.value.Locked.readable = .{};
    }
}
pub inline fn setSizeHint(this: *Response, size_hint: Blob.SizeType) void {
    if (this.priv_body.value == .Locked) {
        this.priv_body.value.Locked.size_hint = size_hint;
        if (this.priv_body.value.Locked.readable.get(this.priv_body.value.Locked.global)) |readable| {
...
pub export fn jsFunctionRequestOrResponseHasBodyValue(_: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) callconv(jsc.conv) jsc.JSValue {
    ...
    if (this_value.as(Response)) |response| {
        return jsc.JSValue.jsBoolean(!response.priv_body.value.isDefinitelyEmpty());
...
pub export fn jsFunctionGetCompleteRequestOrResponseBodyValueAsArrayBuffer(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) callconv(jsc.conv) jsc.JSValue {
    ...
    const body: *Body.Value = brk: {
        if (this_value.as(Response)) |response| {
            break :brk &response.priv_body.value;
        } else if (this_value.as(Request)) |request| {
...
pub fn header(this: *const Response, name: bun.webcore.FetchHeaders.HTTPHeaderName) ?[]const u8 {
    return if (try (this.priv_init.headers orelse return null).fastGet(name)) |str|
        str.slice()
    else
        null;
}
...
pub fn writeFormat(this: *Response, comptime Formatter: type, formatter: *Formatter, writer: anytype, comptime enable_ansi_colors: bool) !void {
    ...
    try writer.print("Response ({f}) {{\n", .{bun.fmt.size(this.priv_body.len(), .{})});

    {
        ...
        try writer.print(comptime Output.prettyFmt("<r><b>{f}<r>", enable_ansi_colors), .{this.priv_url});
        try writer.writeAll("\"");
        try formatter.printComma(Writer, writer, enable_ansi_colors);
        try writer.writeAll("\n");
        ...
        try formatter.printAs(.Double, Writer, writer, jsc.JSValue.jsNumber(this.priv_init.status_code), .NumberObject, enable_ansi_colors);
        try formatter.printComma(Writer, writer, enable_ansi_colors);
        try writer.writeAll("\n");

        try formatter.writeIndent(Writer, writer);
        try writer.writeAll(comptime Output.prettyFmt("<r>statusText<d>:<r> ", enable_ansi_colors));
        try writer.print(comptime Output.prettyFmt("<r>\"<b>{f}<r>\"", enable_ansi_colors), .{this.priv_init.status_text});
        try formatter.printComma(Writer, writer, enable_ansi_colors);
        try writer.writeAll("\n");
        ...
        try formatter.printAs(.Boolean, Writer, writer, jsc.JSValue.jsBoolean(this.priv_redirected), .BooleanObject, enable_ansi_colors);
        try formatter.printComma(Writer, writer, enable_ansi_colors);
        try writer.writeAll("\n");

        formatter.resetLine();
        try this.priv_body.writeFormat(Formatter, formatter, writer, enable_ansi_colors);
    }
    try writer.writeAll("\n");
    try formatter.writeIndent(Writer, writer);
    try writer.writeAll("}");
...
pub fn isOK(this: *const Response) bool {
    return this.priv_init.status_code >= 200 and this.priv_init.status_code <= 299;
}
...
pub fn getURL(
    ...
    return this.priv_url.toJS(globalThis);
}
...
pub fn getResponseType(
    ...
    if (this.priv_init.status_code < 200) {
...
pub fn getStatusText(
    ...
    return this.priv_init.status_text.toJS(globalThis);
}
...
pub fn getRedirected(
    ...
    return JSValue.jsBoolean(this.priv_redirected);
}
...
fn getOrCreateHeaders(this: *Response, globalThis: *jsc.JSGlobalObject) bun.JSError!*FetchHeaders {
    if (this.priv_init.headers == null) {
        this.priv_init.headers = FetchHeaders.createEmpty();

        if (this.priv_body.value == .Blob) {
            const content_type = this.priv_body.value.Blob.content_type;
            if (content_type.len > 0) {
                try this.priv_init.headers.?.put(.ContentType, content_type, globalThis);
            }
        }
    }
...
pub fn doClone(
    ...
    // At this point, this.priv_body.value.Locked.readable still holds the teed stream
    // because checkBodyStreamRef hasn't been called on the original response yet.
    if (this.priv_body.value == .Locked) {
        if (this.priv_body.value.Locked.readable.get(globalThis)) |readable| {
    ...
    var body = brk: {
        if (this.priv_js_ref.tryGet()) |js_ref| {
            if (js.gc.stream.get(js_ref)) |stream| {
                ...
                if (readable != null) {
                    break :brk try this.priv_body.cloneWithReadableStream(globalThis, &readable.?);
                }
            }
        }

        break :brk try this.priv_body.clone(globalThis);
    };
    errdefer body.deinit(bun.default_allocator);
    var _init = try this.priv_init.clone(globalThis);
    errdefer _init.deinit(bun.default_allocator);
    return Response{
        .priv_body = body,
        .priv_init = _init,
        .priv_url = this.priv_url.clone(),
        .priv_redirected = this.priv_redirected,
    };
}
...
pub fn getStatus(
    ...
    return JSValue.jsNumber(this.priv_init.status_code);
}

fn destroy(this: *Response) void {
    this.priv_init.deinit(bun.default_allocator);
    this.priv_body.deinit(bun.default_allocator);
    this.priv_url.deref();
    this.priv_js_ref.deinit();
...
pub fn getContentType(
    ...
    if (this.priv_body.value == .Blob) {
        if (this.priv_body.value.Blob.content_type.len > 0)
            return ZigString.Slice.fromUTF8NeverFree(this.priv_body.value.Blob.content_type);
    }
...
pub fn constructJSON(
    ...
    var response = Response{
        ...
        .priv_init = Response.Init{
            .status_code = 200,
        },
        .priv_url = bun.String.empty,
    };
    var did_succeed = false;
    defer {
        if (!did_succeed) {
            response.priv_body.deinit(bun.default_allocator);
            response.priv_init.deinit(bun.default_allocator);
        }
    }
    ...
    if (@intFromEnum(json_value) != 0) {
        ...
        if (!str.isEmpty()) {
            if (str.value.WTFStringImpl.toUTF8IfNeeded(bun.default_allocator)) |bytes| {
                ...
                response.priv_body.value = Body.Value{
    ...
    if (args.nextEat()) |arg_init| {
        if (arg_init.isUndefinedOrNull()) {} else if (arg_init.isNumber()) {
            response.priv_init.status_code = @as(u16, @intCast(@min(@max(0, arg_init.toInt32()), std.math.maxInt(u16))));
        } else {
            if (Response.Init.init(globalThis, arg_init) catch |err| if (err == error.JSError) return .zero else null) |_init| {
                response.priv_init = _init;
            }
        }
    }
...
pub fn constructRedirectImpl(
    ...
    var response: Response = brk: {
        ...
        if (args.nextEat()) |arg_init| {
            if (arg_init.isUndefinedOrNull()) {} else if (arg_init.isNumber()) {
                response.priv_init.status_code = try validateRedirectStatusCode(globalThis, arg_init.toInt32());
            } else if (try Response.Init.init(globalThis, arg_init)) |_init| {
                errdefer response.priv_init.deinit(bun.default_allocator);
                response.priv_init = _init;

                if (_init.status_code != 200) {
                    response.priv_init.status_code = try validateRedirectStatusCode(globalThis, _init.status_code);
                }
            }
        }
    ...
    response.priv_init.headers = try response.getOrCreateHeaders(globalThis);
    var headers_ref = response.priv_init.headers.?;
...
pub fn constructError(
    ...
    response.priv_js_ref = .initWeak(js_value);
    return js_value;
}
...
pub fn constructor(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame, js_this: jsc.JSValue) bun.JSError!*Response {
    ...
    if (response.priv_body.value == .Blob and
        response.priv_init.headers != null and
        response.priv_body.value.Blob.content_type.len > 0 and
        !response.priv_init.headers.?.fastHas(.ContentType))
    {
        try response.priv_init.headers.?.put(.ContentType, response.priv_body.value.Blob.content_type, globalThis);
    }
    ...
    pub fn init(globalThis: *JSGlobalObject, response_init: jsc.JSValue) bun.JSError!?Init {
        ...
        if (js_type == .DOMWrapper) {
            ...
            if (response_init.asDirect(Response)) |resp| {
                return try resp.priv_init.clone(globalThis);
            }
        }
