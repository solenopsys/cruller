...
priv_heap: if (safety_checks) Owned(*DebugHeap) else *mimalloc.Heap,
...
pub const Borrowed = struct {
    priv_heap: BorrowedHeap,

    pub fn allocator(self: Borrowed) std.mem.Allocator {
        return .{ .ptr = self.priv_heap, .vtable = &heap_allocator_vtable };
    }
    ...
    fn fromOpaque(ptr: *anyopaque) Borrowed {
        return .{ .priv_heap = @ptrCast(@alignCast(ptr)) };
    }

    fn getMimallocHeap(self: Borrowed) *mimalloc.Heap {
        return if (comptime safety_checks) self.priv_heap.inner else self.priv_heap;
    }

    fn assertThreadLock(self: Borrowed) void {
        if (comptime safety_checks) self.priv_heap.thread_lock.assertLocked();
    }
...
pub fn borrow(self: Self) Borrowed {
    return .{ .priv_heap = if (comptime safety_checks) self.priv_heap.get() else self.priv_heap };
}
...
pub fn init() Self {
    ...
    if (comptime !safety_checks) return .{ .priv_heap = mimalloc_heap };
    const heap: Owned(*DebugHeap) = .new(.{
        .inner = mimalloc_heap,
        .thread_lock = .initLocked(),
    });
    return .{ .priv_heap = heap };
}
