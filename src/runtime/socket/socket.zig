...
// bzrt-cut: pub const WindowsNamedPipeContext = if (Environment.isWindows) @import("./WindowsNamedPipeContext.zig") else void;
    ...
    fn runEvent(this: *DuplexUpgradeContext) void {
        switch (this.task_event) {
            .StartTLS => {
                ...
                log("DuplexUpgradeContext.startTLS mode={s}", .{@tagName(this.priv_mode)});
                const is_client = this.priv_mode == .client;
...
pub fn jsUpgradeDuplexToTLS(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    var duplexContext = DuplexUpgradeContext.new(.{
        ...
        .owned_ctx = owned_ctx,
        .priv_mode = if (is_server) .duplex_server else .client,
    });
