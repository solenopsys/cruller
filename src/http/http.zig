...
pub var socket_async_http_abort_tracker = bun.compat.AutoArrayHashMapManaged(u32, uws.AnySocket).init(bun.default_allocator);
...
noinline fn sendInitialRequestPayload(this: *HTTPClient, comptime is_first_call: bool, comptime is_ssl: bool, socket: NewHTTPContext(is_ssl).HTTPSocket) !InitialRequestPayloadResult {
    ...
    const writer = &bun.compat.listWriter(&temporary_send_buffer);
