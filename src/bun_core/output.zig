...
pub const Source = struct {
    ...
    var color_depth_once = bun.compat.once(getColorDepthOnce);
...
pub fn isAIAgent() bool {
    const get_is_agent = struct {
        ...
        var once = bun.compat.once(setValue);
...
fn ScopedLogger(comptime tagname: []const u8, comptime visibility: Visibility) type {
    ...
    return struct {
        ...
        var is_visible_once = bun.compat.once(evaluateIsVisible);
...
pub const DebugTimer = struct {
    timer: bun.DebugOnly(bun.compat.Timer) = undefined,

    pub inline fn start() DebugTimer {
        if (comptime Environment.isDebug) {
            return DebugTimer{
                .timer = bun.compat.Timer.start() catch unreachable,
            };
        } else {
            return .{};
        }
