...
pub fn AllocationScopeIn(comptime Allocator: type) type {
    ...
    const BorrowedScope = struct {
        ...
        priv_parent: BorrowedAllocator,
        priv_state: if (enabled) *State else void,

        pub fn allocator(self: Self) std.mem.Allocator {
            return if (comptime enabled)
                .{ .ptr = self.priv_state, .vtable = &vtable }
            else
                bun.allocators.asStd(self.priv_parent);
        }

        pub fn parent(self: Self) BorrowedAllocator {
            return self.priv_parent;
        }
        ...
        /// a `deinit` method.
        pub fn deinit(self: *Self) void {
            bun.memory.deinit(&self.priv_parent);
            self.* = undefined;
        }

        pub fn stats(self: Self) Stats {
            if (comptime !enabled) @compileError("AllocationScope must be enabled");
            const state = self.priv_state.lock();
            defer self.priv_state.unlock();
            return .{
        ...
        pub fn trackExternalAllocation(
            ...
            if (comptime enabled) self.priv_state.trackExternalAllocation(ptr, ret_addr, extra);
        }

        pub fn trackExternalFree(self: Self, slice: anytype, ret_addr: ?usize) FreeError!void {
            return if (comptime enabled) self.priv_state.trackExternalFree(slice, ret_addr);
        }

        pub fn setPointerExtra(self: Self, ptr: *anyopaque, extra: Extra) void {
            if (comptime enabled) self.priv_state.setPointerExtra(ptr, extra);
        }
        ...
        fn downcastImpl(
            ...
            return .{ .priv_parent = new_parent, .priv_state = state };
        }
    ...
    return struct {
        ...
        priv_parent: Allocator,
        priv_state: if (Self.enabled) Owned(*State) else void,
        ...
        pub fn init(parent_alloc: Allocator) Self {
            return .{
                .priv_parent = parent_alloc,
                .priv_state = if (comptime Self.enabled) .new(.init(
        ...
        pub fn borrow(self: Self) Borrowed {
            return .{
                .priv_parent = self.parent(),
                .priv_state = if (comptime Self.enabled) self.priv_state.get(),
            };
        }
        ...
        pub fn deinit(self: *Self) void {
            bun.memory.deinit(&self.priv_parent);
            if (comptime Self.enabled) self.priv_state.deinit();
            self.* = undefined;
        }

        pub fn parent(self: Self) BorrowedAllocator {
            return bun.allocators.borrow(self.priv_parent);
        }
