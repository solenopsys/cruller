...
const Stats = struct {
    ...
    fn init() Stats {
        ...
        stats.start = bun.compat.milliTimestamp();
        return stats;
    }

    /// Time elapsed since start in milliseconds
    fn elapsed(this: *const Stats) i64 {
        return bun.compat.milliTimestamp() - this.start;
    }
...
fn runTests() u8 {
    ...
    for (tests) |t| {
        ...
        const start = bun.compat.milliTimestamp();
        const result = recover.callForTest(t.func);
        const elapsed = bun.compat.milliTimestamp() - start;
