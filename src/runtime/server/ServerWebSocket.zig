...
priv_handler: *WebSocketServer.Handler,
priv_this_value: jsc.JSRef = .empty(),
priv_flags: Flags = .{},
priv_signal: ?*bun.webcore.AbortSignal = null,
...
inline fn websocket(this: *const ServerWebSocket) uws.AnyWebSocket {
    return this.priv_flags.websocket();
}
...
pub fn onOpen(this: *ServerWebSocket, ws: uws.AnyWebSocket) void {
    ...
    this.priv_flags.packed_websocket_ptr = @truncate(@intFromPtr(ws.raw()));
    this.priv_flags.closed = false;
    this.priv_flags.ssl = ws == .ssl;

    var handler = this.priv_handler;
    const vm = this.priv_handler.vm;
    ...
    this.priv_flags.opened = false;

    if (onOpenHandler.isEmptyOrUndefinedOrNull()) {
        return;
    }

    const this_value = this.priv_this_value.tryGet() orelse .js_undefined;
    ...
    this.priv_flags.opened = true;
    if (result.toError()) |err_value| {
        log("onOpen exception", .{});

        if (!this.priv_flags.closed) {
            this.priv_flags.closed = true;
...
pub fn onMessage(
    ...
    const onMessageHandler = this.priv_handler.onMessage;
    if (onMessageHandler.isEmptyOrUndefinedOrNull()) return;
    var globalObject = this.priv_handler.globalObject;
    ...
    const arguments = [_]JSValue{
        this.priv_this_value.tryGet() orelse .js_undefined,
        switch (opcode) {
    ...
    if (result.toError()) |err_value| {
        this.priv_handler.runErrorCallback(vm, globalObject, err_value);
        return;
    }
...
pub inline fn isClosed(this: *const ServerWebSocket) bool {
    return this.priv_flags.closed;
}

pub fn onDrain(this: *ServerWebSocket, _: uws.AnyWebSocket) void {
    log("onDrain", .{});

    const handler = this.priv_handler;
    const vm = handler.vm;
    ...
    if (handler.onDrain != .zero) {
        ...
        var corker = Corker{
            .args = &[_]jsc.JSValue{this.priv_this_value.tryGet() orelse .js_undefined},
            .globalObject = globalObject,
...
fn binaryToJS(this: *const ServerWebSocket, globalThis: *jsc.JSGlobalObject, data: []const u8) bun.JSError!jsc.JSValue {
    return switch (this.priv_flags.binary_type) {
...
pub fn onPing(this: *ServerWebSocket, _: uws.AnyWebSocket, data: []const u8) void {
    ...
    _ = cb.call(
        globalThis,
        .js_undefined,
        &[_]jsc.JSValue{ this.priv_this_value.tryGet() orelse .js_undefined, this.binaryToJS(globalThis, data) catch .zero }, // TODO: properly propagate exception upwards
    ) catch |e| {
...
pub fn onClose(this: *ServerWebSocket, _: uws.AnyWebSocket, code: i32, message: []const u8) void {
    ...
    const signal = this.priv_signal;
    this.priv_signal = null;
    ...
    defer {
        ...
        if (this.priv_this_value.isNotEmpty()) {
            this.priv_this_value.downgrade();
        }
    }
    ...
    if (!handler.onClose.isEmptyOrUndefinedOrNull()) {
        ...
        const message_js = bun.String.createUTF8ForJS(globalObject, message) catch |e| {
            ...
            log("onClose error (message) {}", .{this.priv_this_value.isNotEmpty()});
            handler.runErrorCallback(vm, globalObject, err);
            return;
        };

        _ = handler.onClose.call(globalObject, .js_undefined, &[_]jsc.JSValue{ this.priv_this_value.tryGet() orelse .js_undefined, JSValue.jsNumber(code), message_js }) catch |e| {
            const err = globalObject.takeException(e);
            log("onClose error {}", .{this.priv_this_value.isNotEmpty()});
            handler.runErrorCallback(vm, globalObject, err);
            return;
        };
    } else if (signal) |sig| {
...
pub fn finalize(this: *ServerWebSocket) void {
    log("finalize", .{});
    this.priv_this_value.finalize();
    if (this.priv_signal) |signal| {
        this.priv_signal = null;
...
pub fn publish(
    ...
    const app = this.priv_handler.app orelse {
        log("publish() closed", .{});
        return JSValue.jsNumber(0);
    };
    const flags = this.priv_handler.flags;
    const ssl = flags.ssl;
...
pub fn getData(
    ...
    log("getData()", .{});
    if (this.priv_this_value.tryGet()) |this_value| {
...
pub fn setBinaryType(this: *ServerWebSocket, globalThis: *jsc.JSGlobalObject, value: jsc.JSValue) bun.JSError!void {
    ...
    switch (btype orelse
        ...
        .ArrayBuffer, .Buffer, .Uint8Array => |val| {
            this.priv_flags.binary_type = val;
            return;
        },
        else => {
...
pub fn getRemoteAddress(
    ...
    const address: bun.compat.NetAddress = switch (address_bytes.len) {
        4 => bun.compat.NetAddress.initIp4(address_bytes[0..4].*, 0),
        16 => bun.compat.NetAddress.initIp6(address_bytes[0..16].*, 0, 0, 0),
