        ...
        pub fn save(
            ...
            }
            {
                ...
                const metadata_bytes = brk: {
                    ...
                    var metadata_stream = bun.compat.fixedBufferStream(&metadata_buf);

                    try metadata.encode(metadata_stream.writer());

                    if (comptime bun.Environment.isDebug) {
                        var metadata_stream2 = bun.compat.fixedBufferStream(metadata_buf[0..Metadata.size]);
    ...
    pub fn toFile(
        ...
        const cache_dir_fd = brk: {
            if (std.fs.path.dirname(cache_file_path)) |dirname| {
                var dir = std.Io.Dir.cwd().openDir(bun.compat.io(), dirname, .{ .access_sub_paths = true }) catch {
