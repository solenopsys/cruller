...
fn startNextBundleIfPresent(dev: *DevServer) void {
    ...
    if (dev.next_bundle.reload_event != null or dev.next_bundle.requests.first != null or dev.next_bundle.promise.strong.hasValue()) {
        ...
        } else .{ false, bun.compat.Timer.start() catch @panic("timers unsupported") };
...
fn getOrPutRouteBundle(dev: *DevServer, route: RouteBundle.UnresolvedIndex) !RouteBundle.Index {
    ...
    dev.route_bundles.appendAssumeCapacity(.{
        ...
        .client_script_generation = bun.compat.rand().int(u32),
...
pub const HmrTopic = enum(u8) {
    ...
    pub const Bits = bits: {
        ...
        for (enum_fields, &fields) |e, *s| {
            s.* = .{ .name = e.name, .type = bool, .default_value_ptr = &false, .is_comptime = false, .alignment = 0 };
        }
        const fields_c = fields;
        break :bits bun.meta.ReifyStruct(.@"packed", @Int(.unsigned, max_count), &fields_c);
    };
};
...
fn dumpStateDueToCrash(dev: *DevServer) !void {
    ...
    const filepath = std.fmt.bufPrintZ(&filepath_buf, "incremental-graph-crash-dump.{d}.html", .{bun.compat.timestamp()}) catch "incremental-graph-crash-dump.html";
