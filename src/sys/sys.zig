...
// bzrt-cut:     .windows => @import("../errno/windows_errno.zig"),
    .linux => @import("../errno/linux_errno.zig"),
    else => @compileError("bzrt: linux only"),
// bzrt-cut:     .mac => @import("../errno/darwin_errno.zig"),
// bzrt-cut:     .freebsd => @import("../errno/freebsd_errno.zig"),
    .wasm => {},
};
...
pub fn dlsymWithHandle(comptime Type: type, comptime name: [:0]const u8, comptime handle_getter: fn () ?*anyopaque) ?Type {
    ...
    const Wrapper = struct {
        ...
        var failed = false;
        pub var once = bun.compat.once(loadOnce);
        fn loadOnce() void {
