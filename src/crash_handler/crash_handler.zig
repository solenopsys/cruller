    ...
    nosuspend switch (panic_stage) {
        0 => {
...

            {
                ...
                var writer_w = std.Io.File.stderr().writerStreaming(bun.compat.io(), &.{});
                ...
                if (reason != .out_of_memory or debug_trace) {
                    ...
                    if (bun.cli_compat.is_main_thread) {
                ...
                !bun.isProcessReloadInProgressOnAnotherThread())
            {
                ...
                Output.prettyErrorln("<d>--- Bun is auto-restarting due to crash <d>[time: <b>{d}<r><d>] ---<r>", .{
                    @max(bun.compat.milliTimestamp(), 0),
                });
                Output.flush();
        ...
        inline 1, 2 => |t| {
            ...
            // call abort()
            var stderr_w = std.Io.File.stderr().writerStreaming(bun.compat.io(), &.{});
    ...
    switch (err) {
        ...
        error.SystemFdQuotaExceeded => {
            if (comptime bun.Environment.isPosix) {
                const limit = if (std.posix.getrlimit(.NOFILE)) |limit| limit.cur else null;
...
fn encodeTraceString(opts: TraceString, writer: anytype) !void {
    ...
    try writer.writeByte(if (bun.cli_compat.cmd) |cmd| cmd.char() else '_');
...
pub fn dumpCurrentStackTrace(first_address: ?usize, limits: WriteStackTraceLimits) void {
    ...
    const captured = std.debug.captureCurrentStackTrace(.{ .first_address = first_address orelse @returnAddress() }, &addrs);
    const stack: std.builtin.StackTrace = .{ .index = captured.return_addresses.len, .instruction_addresses = &addrs };
...
pub const StoredTrace = struct {
    ...
    pub fn capture(begin: ?usize) StoredTrace {
        ...
        const captured = std.debug.captureCurrentStackTrace(.{ .first_address = begin orelse @returnAddress() }, frame.instruction_addresses);
        frame.index = captured.return_addresses.len;
...
fn printLineFromFileAnyOs(out_stream: anytype, tty_config: std.io.tty.Config, source_location: SourceLocation) !void {
    ...
    var fbs = bun.compat.fixedBufferStream(&line_buf);
    read_line: {
