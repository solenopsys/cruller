...
origin_timer: bun.compat.Timer = undefined,
...
pub fn initWithModuleGraph(
    ...
    vm.* = VirtualMachine{
        ...
        .origin_timer = bun.compat.Timer.start() catch @panic("Timers are not supported on this system."),
...
pub fn init(opts: Options) !*VirtualMachine {
    ...
    vm.* = VirtualMachine{
        ...
        .macros = MacroMap.init(allocator),
        .macro_entry_points = @TypeOf(vm.macro_entry_points).init(allocator),
        .origin_timer = bun.compat.Timer.start() catch @panic("Please don't mess with timers."),
        .origin_timestamp = getOriginTimestamp(),
        .ref_strings = jsc.RefString.Map.init(allocator),
        .ref_strings_mutex = .{},
        .debug_thread_id = if (Environment.allow_assert) std.Thread.getCurrentId(),
...
// bzrt-cut: const PackageManager = @import("../install/install.zig").PackageManager;
