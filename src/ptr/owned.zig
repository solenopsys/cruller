    ...
    return struct {
        ...
        pub fn allocDupeIn(data: ConstPointer, allocator_: Allocator) AllocError!Self {
            ...
            return switch (comptime info.kind()) {
                ...
                .slice => .{
                    .priv_pointer = try bun.allocators.asStd(allocator_).dupe(Child, unwrapped),
                    .priv_allocator = allocator_,
                },
            };
        }
        ...
        pub fn fromRawIn(data: Pointer, allocator_: Allocator) Self {
            return .{
                ...
                .priv_allocator = if ((comptime info.isOptional()) and data == null)
                    undefined
                else
                    allocator_,
            };
        ...
        pub fn intoRaw(self: *Self) Pointer {
            ...
            if ((comptime !info.isOptional()) or self.priv_pointer != null) {
        ...
        pub fn intoRawWithAllocator(self: *Self) PointerAndAllocator {
            ...
            const data = if (comptime info.isOptional())
                self.priv_pointer orelse return null
            else
                self.priv_pointer;
            return .{ data, self.priv_allocator };
        }
        ...
        pub const take = if (info.isOptional()) struct {
            ...
            pub fn take(self: *Self) ?OwnedNonOptional {
                defer self.* = .initNull();
                return .{
                    .priv_pointer = self.priv_pointer orelse return null,
                    .priv_allocator = self.priv_allocator,
                };
            }
        }.take;
        ...
        pub const toOptional = if (!info.isOptional()) struct {
            ...
            pub fn toOptional(self: *Self) OwnedOptional {
                defer self.* = undefined;
                return .{
                    .priv_pointer = self.priv_pointer,
                    .priv_allocator = self.priv_allocator,
                };
            }
        }.toOptional;
        ...
        pub fn toDynamic(self: *Self) owned.Dynamic(Pointer) {
            ...
            defer self.* = undefined;
            const data = if (comptime info.isOptional())
                self.priv_pointer orelse return .initNull()
            else
                self.priv_pointer;
            defer bun.memory.deinit(&self.priv_allocator);
        ...
        pub fn allocator(self: Self) MaybeAllocator {
            return if ((comptime info.isOptional()) and self.priv_pointer == null)
                null
            else
                bun.allocators.borrow(self.priv_allocator);
        }
        ...
        fn deinitImpl(self: *Self, comptime mode: enum { deep, shallow }) void {
            defer self.* = undefined;
            const data = if (comptime info.isOptional())
                self.priv_pointer orelse return
            else
                self.priv_pointer;
