...
pub const Reader = bun.compat.GenericReader(File, anyerror, stdIoRead);
...
pub const Writer = bun.compat.GenericWriter(File, anyerror, stdIoWrite);
pub const QuietWriter = if (Environment.isDebug) bun.compat.GenericWriter(File, anyerror, stdIoWriteQuietDebug) else Writer;
...
pub fn isTty(self: File) bool {
    return bun.compat.isatty(self.handle.cast());
}
