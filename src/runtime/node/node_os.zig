...
fn cpusImplLinux(globalThis: *jsc.JSGlobalObject) !jsc.JSValue {
    ...
    {
        const file = std.Io.Dir.cwd().openFile(bun.compat.io(), "/proc/stat", .{}) catch {
...
fn networkInterfacesPosix(globalThis: *jsc.JSGlobalObject) bun.JSError!jsc.JSValue {
    ...
    while (it) |iface| : (it = iface.ifa_next) {
        ...
        const addr = bun.compat.NetAddress.initPosix(@alignCast(@as(*std.posix.sockaddr, @ptrCast(iface.ifa_addr))));
        const netmask = bun.compat.NetAddress.initPosix(@alignCast(@as(*std.posix.sockaddr, @ptrCast(iface.ifa_netmask))));
    ...
    for (ifaces[0..@intCast(count)]) |iface| {
        ...
        var cidr = jsc.JSValue.null;
        {
            ...
            const addr_str = bun.fmt.formatIp(
                // bun.compat.NetAddress will do ptrCast depending on the family so this is ok
                bun.compat.NetAddress.initPosix(@ptrCast(&iface.address.address4)),
                &ip_buf,
            ) catch unreachable;
            if (maybe_suffix) |suffix| {
                //NOTE addr_str might not start at buf[0] due to slicing in formatIp
        ...
        // netmask
        {
            const str = bun.fmt.formatIp(
                // bun.compat.NetAddress will do ptrCast depending on the family so this is ok
                bun.compat.NetAddress.initPosix(@ptrCast(&iface.netmask.netmask4)),
                &ip_buf,
            ) catch unreachable;
            interface.put(globalThis, jsc.ZigString.static("netmask"), jsc.ZigString.init(str).withEncoding().toJS(globalThis));
        }
...
pub fn uptime(global: *jsc.JSGlobalObject) bun.JSError!f64 {
    switch (Environment.os) {
        ...
        .mac, .freebsd => {
            ...
            return @floatFromInt(bun.compat.timestamp() - boot_time.sec);
        },
        .linux => {
            var info: c.struct_sysinfo = undefined;
            if (c.sysinfo(&info) == 0)
