    ...
    if (comptime strings.eqlComptime(lookup_name, "a") or strings.eqlComptime(lookup_name, "aaaa")) {
        ...
        while (this.hostent.h_addr_list.?[count]) |addr| : (count += 1) {
            const addrString = (if (this.hostent.h_addrtype == c_ares.AF.INET6)
                bun.dns.addressToJS(&bun.compat.NetAddress.initIp6(addr[0..16].*, 0, 0, 0), globalThis)
            else
                bun.dns.addressToJS(&bun.compat.NetAddress.initIp4(addr[0..4].*, 0), globalThis)) catch return globalThis.throwOutOfMemoryValue();
                ...
                try GetAddrInfo.Result.toJS(
                    &.{
                        .address = switch (this_node.family) {
                            c_ares.AF.INET => bun.compat.NetAddress{ .in = .{ .sa = bun.cast(*const std.posix.sockaddr.in, this_node.addr.?).* } },
                            c_ares.AF.INET6 => bun.compat.NetAddress{ .in6 = .{ .sa = bun.cast(*const std.posix.sockaddr.in6, this_node.addr.?).* } },
                            else => unreachable,
                        },
                        .ttl = this_node.ttl,
                    },
