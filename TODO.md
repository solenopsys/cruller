...
- `x64-musl` — built in `musl.Containerfile`, verified with Alpine's musl loader.
- `aarch64-gnu`
- `aarch64-musl`
...
- Host builds still need system `clang` — `scripts/build/compile.ts` shells out to
  ...
  needed). The supported musl build path is the native Alpine builder in
  `musl.Containerfile`.
...
- The Alpine image currently covers `x64-musl`; add the aarch64 buildx
  variant once its WebKit artifact and native builder path are available.
