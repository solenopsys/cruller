...
pub fn LinearFifo(
    ...
    return struct {
        ...
        const Self = @This();
        pub const Reader = bun.compat.GenericReader(*Self, error{}, readFn);
        pub const Writer = bun.compat.GenericWriter(*Self, error{OutOfMemory}, appendWrite);
...
test "LinearFifo(u8, .Dynamic)" {
...

    {
        ...
        var in_fbs = bun.compat.fixedBufferStream("pump test");
        var out_buf: [50]u8 = undefined;
        var out_fbs = bun.compat.fixedBufferStream(&out_buf);
