...
pub fn BabyList(comptime Type: type) type {
    ...
    return struct {
        ...
        len: u32 = 0,
        cap: u32 = 0,
        priv_origin: if (safety_checks) Origin else void = if (safety_checks) .owned,
        priv_allocator: bun.safety.CheckedAllocator = .{},
        ...
        pub fn initCapacity(allocator: std.mem.Allocator, len: usize) OOM!Self {
            ...
            this.priv_allocator.set(allocator);
            return this;
        }
        ...
        pub fn moveFromList(list_ptr: anytype) Self {
            ...
            if (comptime @TypeOf(allocator) == void) {
                ...
                this.priv_allocator.set(bun.allocators.asStd(allocator));
        ...
        pub fn appendFmt(
            ...
            if ((comptime safety_checks) and this.len == this.cap) this.assertOwned();
            var list_ = this.listManaged(allocator);
            const writer = bun.compat.listWriter(&list_);
        ...
        pub fn fromBorrowedSliceDangerous(items: []const Type) Self {
            ...
            if (comptime safety_checks) this.priv_origin = .{ .borrowed = .{
        ...
        pub fn transferOwnership(this: *Self, new_allocator: anytype) void {
            this.priv_allocator.transferOwnership(new_allocator);
        }
