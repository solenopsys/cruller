...
priv_headers: ?*FetchHeaders = null,
signal: ?*AbortSignal = null,
priv_body: *Body.Value.HiveRef,
...
pub fn memoryCost(this: *const Request) usize {
    return @sizeOf(Request) + this.request_context.memoryCost() + this.url.byteSlice().len + this.priv_body.value.memoryCost();
}
...
pub fn calculateEstimatedByteSize(this: *Request) void {
    this.reported_estimated_size = this.priv_body.value.estimatedSize() + this.sizeOfURL() + @sizeOf(Request);
}
...
pub fn writeFormat(this: *Request, this_value: JSValue, comptime Formatter: type, formatter: *Formatter, writer: anytype, comptime enable_ansi_colors: bool) !void {
    ...
    try writer.print("{s} ({f}) {{\n", .{ class_label, bun.fmt.size(this.priv_body.value.size(), .{}) });
    {
        formatter.indent += 1;
        ...
        if (this.priv_body.value == .Blob) {
            try writer.writeAll("\n");
            try formatter.writeIndent(Writer, writer);
            try this.priv_body.value.Blob.writeFormat(Formatter, formatter, writer, enable_ansi_colors);
        } else if (this.priv_body.value == .InternalBlob or this.priv_body.value == .WTFStringImpl) {
            try writer.writeAll("\n");
            try formatter.writeIndent(Writer, writer);
            const size = this.priv_body.value.size();
            if (size == 0) {
        ...
        } else if (this.priv_body.value == .Locked) {
...
pub fn mimeType(this: *const Request) string {
    ...
    switch (this.priv_body.value) {
        ...
        .InternalBlob => return this.priv_body.value.InternalBlob.contentType(),
        .WTFStringImpl => return MimeType.text.value,
        // .InlineBlob => return this.priv_body.value.InlineBlob.contentType(),
...
pub fn getReferrer(
    ...
    if (this.priv_headers) |headers_ref| {
...
pub fn constructInto(globalThis: *jsc.JSGlobalObject, arguments: []const jsc.JSValue, this_value: jsc.JSValue) bun.JSError!Request {
    ...
    var req = Request{
        .priv_body = body,
        .priv_js_ref = .initWeak(this_value),
    };
    defer {
        if (!success) {
            req.finalizeWithoutDeinit();
            _ = req.priv_body.unref();
        }
        if (req.priv_body != body) {
            _ = body.unref();
        }
    }
    ...
    for (values_to_try) |value| {
        ...
        if (value_type == .DOMWrapper) {
            if (value.asDirect(Request)) |request| {
                ...
                if (!fields.contains(.body)) {
                    switch (request.priv_body.value) {
                        .Null, .Empty, .Used => {},
                        else => {
                            req.priv_body.value = try request.priv_body.value.clone(globalThis);
                            fields.insert(.body);
                        },
                    }
                }
            ...
            if (value.asDirect(Response)) |response| {
                ...
                if (!fields.contains(.headers)) {
                    if (response.getInitHeaders()) |headers| {
                        req.priv_headers = try headers.cloneThis(globalThis);
                        fields.insert(.headers);
                    }
                }
                ...
                if (!fields.contains(.body)) {
                    ...
                    switch (bodyValue.*) {
                        .Null, .Empty, .Used => {},
                            ...
                            req.priv_body.value = try bodyValue.clone(globalThis);
                            fields.insert(.body);
                        },
                    }
                }
        ...
        if (!fields.contains(.body)) {
            if (try value.fastGet(globalThis, .body)) |body_| {
                fields.insert(.body);
                req.priv_body.value = try Body.Value.fromJS(globalThis, body_);
            }
    ...
    if (req.priv_body.value == .Blob and
        req.priv_headers != null and
        req.priv_body.value.Blob.content_type.len > 0 and
        !req.priv_headers.?.fastHas(.ContentType))
    {
        try req.priv_headers.?.put(.ContentType, req.priv_body.value.Blob.content_type, globalThis);
    }
...
pub fn setFetchHeaders(
    ...
    if (this.priv_headers) |old_headers| {
        old_headers.deref();
    }

    this.priv_headers = headers;
}
...
pub fn ensureFetchHeaders(
    ...
    if (this.request_context.getRequest()) |req| {
        ...
        this.priv_headers = FetchHeaders.createFromUWS(req);
    } else {
        // we don't have a request context, so we need to create an empty headers object
        this.priv_headers = FetchHeaders.createEmpty();
        const content_type = switch (this.priv_body.value) {
        ...
        if (content_type) |content_type_| {
            if (content_type_.len > 0) {
                try this.priv_headers.?.put(.ContentType, content_type_, globalThis);
            }
        }
    }

    return this.priv_headers.?;
}

pub fn getFetchHeadersUnlessEmpty(
    this: *Request,
) ?*FetchHeaders {
    if (this.priv_headers == null) {
        if (this.request_context.getRequest()) |req| {
            // we have a request context, so we can get the headers from it
            this.priv_headers = FetchHeaders.createFromUWS(req);
        }
    ...
    const headers = this.priv_headers orelse return null;
    if (headers.isEmpty()) {
        return null;
    }
    return headers;
...
pub fn cloneHeaders(this: *Request, globalThis: *JSGlobalObject) bun.JSError!?*FetchHeaders {
    if (this.priv_headers == null) {
        if (this.request_context.getRequest()) |uws_req| {
            this.priv_headers = FetchHeaders.createFromUWS(uws_req);
        }
    }

    if (this.priv_headers) |head| {
        if (head.isEmpty()) {
            return null;
        }
    ...
    var body_ = brk: {
        if (this.priv_js_ref.tryGet()) |js_ref| {
            if (js.gc.stream.get(js_ref)) |stream| {
                ...
                if (readable != null) {
                    break :brk try this.priv_body.value.cloneWithReadableStream(globalThis, &readable.?);
                }
            }
        }

        break :brk try this.priv_body.value.clone(globalThis);
    };
