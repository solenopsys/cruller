...
priv_ref_count: bun.ptr.RawRefCount(u32, .single_threaded) = .init(0),
...
pub fn onStructuredCloneSerialize(
    ...
    const Writer = bun.compat.GenericWriter(StructuredCloneWriter, StructuredCloneWriter.WriteError, StructuredCloneWriter.write);
    const writer = Writer{
        .context = .{
            .ctx = ctx,
            .impl = writeBytes,
...
pub fn onStructuredCloneDeserialize(globalThis: *jsc.JSGlobalObject, ptr: *[*]u8, end: [*]u8) bun.JSError!JSValue {
    ...
    var buffer_stream = bun.compat.fixedBufferStream(ptr.*[0..total_length]);
...
pub fn JSDOMFile__construct_(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!*Blob {
    ...
    if (!set_last_modified) {
        ...
        blob.last_modified = @floatFromInt(bun.compat.milliTimestamp());
    }
...
pub fn isHeapAllocated(self: *const Blob) bool {
    return self.priv_ref_count.raw_value != 0;
}

fn setNotHeapAllocated(self: *Blob) void {
    self.priv_ref_count = .init(0);
}
...
export fn Blob__ref(self: *Blob) void {
    ...
    self.priv_ref_count.increment();
}

export fn Blob__deref(self: *Blob) void {
    bun.assertf(self.isHeapAllocated(), "cannot deref: this Blob is not heap-allocated", .{});
    if (self.priv_ref_count.decrement() == .should_destroy) {
        self.priv_ref_count.increment(); // deinit has its own isHeapAllocated() guard around bun.destroy(this), so this is needed to ensure that returns true.
        self.deinit();
    }
}
...
// bzrt-cut: const Archive = @import("../api/Archive.zig");
