...
test "V8 executes source buffers through Engine" {
...
test "V8 reports script exceptions through Engine.run" {
    var harness: Harness = undefined;
    try harness.init(std.testing.allocator);
    defer harness.deinit();
...
test "V8 handles bundled SSR requests through command and data streams" {
    const source =
