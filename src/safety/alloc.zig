...
pub const CheckedAllocator = struct {
    ...
    priv_allocator: if (enabled) NullableAllocator else void = if (enabled) .init(null),
    priv_trace: if (traces_enabled) StoredTrace else void = if (traces_enabled) StoredTrace.empty,
    ...
    pub fn set(self: *Self, alloc: Allocator) void {
        if (comptime !enabled) return;
        if (self.priv_allocator.isNull()) {
            self.priv_allocator = .init(alloc);
            if (comptime traces_enabled) {
                self.priv_trace = StoredTrace.capture(@returnAddress());
            }
        } else {
            self.assertEq(alloc);
        }
    ...
    pub fn assertEq(self: Self, alloc: Allocator) void {
        if (comptime !enabled) return;
        const old_alloc = self.priv_allocator.get() orelse return;
    ...
    pub inline fn transferOwnership(self: *Self, new_allocator: anytype) void {
        ...
        const old_allocator = self.priv_allocator.get() orelse return;
